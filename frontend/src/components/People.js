...
import PersonInsertForm from "./PersonInsertForm";
...
export default function People() {
  ...
  const handleFormSubmit = async (updatedPerson) => {
    ...
    const response = await fetch(
      `http://localhost:8080/people/${updatedPerson.id}`,
      requestOptions
    );
  ...
  }, [updatingPerson, isDeleted, newPerson]);
  ...
  return (
    <article className="container">
      ...
      <button onClick={() => handleInsertClick()}>Insert new person</button>
